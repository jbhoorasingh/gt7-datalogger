...
| GET | `/api/laps` | all lap summaries, newest first. `?track=` filters by the session's circuit, `?category=` by the lap's car class; summaries carry `track_name`, `race_position` (position when the lap completed; −1 = no position reporting) and a `salvaged` flag ([replay-salvaged](../internals/lap-detection.md#replay-salvage) laps stay traceable everywhere their time shows up). The list never reads the 60 Hz sample blobs, so querying the whole archive — which is what the Bests board and the Analysis *Add lap…* picker do — stays cheap however large it grows |
