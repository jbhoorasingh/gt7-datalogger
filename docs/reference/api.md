...
| PUT | `/api/admin/settings` | `{ps_ip?, source?, log_level?, webhook_url?, webhook_events?, packet_format?, race_engineer?, race_engineer_verbosity?, race_engineer_categories?, race_engineer_units?, sync_url?, sync_token?, sync_enabled?, sync_tracks?}` — applied live, persisted to the DB. `sync_url` is the server address as typed — a bare host is read as `https://`, `http://` and `gt7sync+http://` are kept as http, empty restores the default server — or a whole `gt7sync://host/?token=…` connection string, which sets the token too and is never stored or echoed as-is; `sync_token` sets the account token on its own (one printable line; empty forgets it). GET answers with `sync_url`, `sync_token_set` and a `sync_token_hint` (`…a1b2`), never the token. 400 for a malformed address, token or string. Changing the server drops the capabilities and the record of what it accepted; the token is left alone |
| GET | `/api/admin/sync` | where the [sync client](../guide/admin.md#sync) stands: `{configured, url, token_hint, enabled, capabilities, capabilities_error, checked_at, types: {<type>: {description, enabled, offered, supported, active, state, error, last_ok_at, last_attempt_at, uploads, queued, tracks: {<status>: count}}}}`. `offered` is null until the server has been asked; `state` is `off`, `idle`, `syncing`, `error` or `unsupported` (the server offers a type this build cannot send) |
...
| POST | `/api/admin/sync/push` | send every eligible bundle now, without waiting for it to settle and clearing any retry backoff; bundles the server already accepted unchanged are still skipped. 400 unless track sync is active |
