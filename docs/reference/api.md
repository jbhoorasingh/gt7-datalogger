...
| POST | `/api/track-bundles/shared/{slug}/pull` | fetch that bundle from the shared repo and merge it through exactly the import path below (same validation, same voting merge, same `corners_kept` protection). The server re-reads the index and follows only the URL it maps the slug to — the client never supplies one. `?track=` overrides the label, same as import |
